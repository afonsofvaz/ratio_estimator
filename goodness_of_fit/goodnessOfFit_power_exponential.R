...
ntg=200
mu=c(1,5)
shift.grid=seq(-0.5,8,length.out = 40)
ntot=ntr+ntg
...
for(b in 1:B)
{
  ...
  for(s in seq_along(shift.grid))
  {
    ...
    # tr:
    ytr=runif(ntr)<p.tg
    xtr=rexp(ntr,mu[ytr+1])
    #hist(xtr)
    
    
    # tg:
    ytg=runif(ntg)<p.tg
    xtg=rexp(ntg,mu.shift[ytg+1])
    #hist(xtg)
    ...
    p[s,b]=result
    saveRDS(list(p=p,mu=mu,shift.grid=shift.grid),"goodness_of_fit/outputs/results/powerMC_exponential.RDS")
  }
