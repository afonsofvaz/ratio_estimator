...
results <- readRDS("powerMC_gaussian.RDS")
...
results <- readRDS("powerMC_exponential.RDS")
mean=apply(results$p,1,function(x)mean(x<0.05,na.rm=T))
se=sqrt(apply(results$p,1,function(x){
  mean(x<0.05,na.rm=T)*mean(x>=0.05,na.rm=T)/sum(!is.na(x))
}))
...
results <- readRDS("powerMC_normal_exponential.RDS")
mean=apply(results$p,1,function(x)mean(x<0.05,na.rm=T))
se=sqrt(apply(results$p,1,function(x){
  mean(x<0.05,na.rm=T)*mean(x>=0.05,na.rm=T)/sum(!is.na(x))
}))
...
results <- readRDS("powerMC_beta.RDS")
mean=apply(results$p,1,function(x)mean(x<0.05,na.rm=T))
se=sqrt(apply(results$p,1,function(x){
  mean(x<0.05,na.rm=T)*mean(x>=0.05,na.rm=T)/sum(!is.na(x))
}))
...
eps <- 0.01
pdf("../Figures/power_all.pdf",width = 10,height = 7)
