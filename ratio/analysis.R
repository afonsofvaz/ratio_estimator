...
## cancer
load("ratio/outputs/results/out.cancer.ratio.RData")
load("ratio/outputs/results/out.cancer.comb.new.RData")
load("ratio/outputs/results/out.cancer.em.RData")
out.cancer.ratio$Dataset = rep("cancer",nrow(out.cancer.ratio))
out.cancer.comb.new$Dataset = rep("cancer",nrow(out.cancer.comb.new))
out.cancer.em$Dataset = rep("cancer",nrow(out.cancer.em))

## block
load("ratio/outputs/results/out.block.ratio.RData")
load("ratio/outputs/results/out.block.comb.new.RData")
load("ratio/outputs/results/out.block.em.RData")
...
## bank
load("ratio/outputs/results/out.bank.ratio.RData")
load("ratio/outputs/results/out.bank.comb.new.RData")
load("ratio/outputs/results/out.bank.em.RData")
...
## spam
load("ratio/outputs/results/out.spam.ratio.RData")
load("ratio/outputs/results/out.spam.comb.new.RData")
load("ratio/outputs/results/out.spam.em.RData")
...
dataratio = rbind(out.spam.ratio,out.cancer.ratio,out.block.ratio,out.bank.ratio,
                  out.cancer.em,out.bank.em,out.block.em,out.spam.em)

datacomb = rbind(out.spam.comb.new,out.cancer.comb.new,out.block.comb.new,out.bank.comb.new)
...
ggsave(g1,file = "ratio/outputs/plots/error_ratio_all.png",width = 12,height = 8,limitsize = FALSE)
...
ggsave(g2,file = "ratio/outputs/plots/error_ratio.png",width = 12,height = 8,limitsize = FALSE)
...
tab_min$aux2 = rep(NA,nrow(tab_min))
...
g3 = tab_min %>% group_by(theta_tg,aux2) %>% summarise(freq = sum(Ind)) %>% 
...
ggsave(g3,file = "ratio/outputs/plots/counting_theta.pdf",width = 10,height = 8,limitsize = FALSE)


g4 = tab_min %>% group_by(Dataset,aux2) %>% summarise(freq = sum(Ind)) %>% 
  ggplot(aes(x = aux2, y = freq)) + geom_bar(stat="identity") + facet_grid(. ~ Dataset) + theme_bw(base_size = 14) +
  theme(axis.text.x = element_text(angle = 90))  + xlab("Method type")  +   ylab("Frequency")
ggsave(g4,file = "ratio/outputs/plots/counting_dataset.pdf",width = 10,height = 8,limitsize = FALSE)


tab_bar = tab_min %>% group_by(Method) %>% summarise(freq = sum(Ind)) %>% 
  arrange(desc(freq)) 
tab_bar$Method = factor(tab_bar$Method,levels = tab_bar$Method)

g5 = tab_bar %>% ggplot(aes(x = Method, y = freq)) + geom_bar(stat="identity") + theme_bw(base_size = 14) +
  theme(axis.text = element_text(angle = 90,size = 16),axis.title.x = element_text(size=16),
        axis.title.y = element_text(size=18)) +
  ylab("Frequency")
ggsave(g5,file = "ratio/outputs/plots/couting.pdf",width = 10,height = 8,limitsize = FALSE)
...
coverage = dataratio %>% dplyr::filter(!(method %in% c("CC-LR","CC-knn","CC-RF","EM-RF","EM-LR","EM-knn"))) %>% 
  dplyr::group_by(Dataset,theta_tg) %>% 
  dplyr::summarise(Coverage = mean(ind_int))  

pdf("ratio/outputs/plots/coverage.pdf")
...
## Combined
datacomb$method[datacomb$method == "kernel"] = "kernel.gaussian"
datacomb$Dataset = factor(datacomb$Dataset,levels = names.ordered)
...
pdf("ratio/outputs/plots/comb.pdf", width = 12,height = 9)
